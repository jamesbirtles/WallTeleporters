...
public class BlockWallTeleporterBase extends BlockContainer
{
	...
	public static int connectedSidesForDelta(int side, IBlockAccess world, int x, int y, int z, int dx, int dy, int dz)
	{
		...
		if(tileEntity instanceof TileWallTeleporter)
		{
			...
			{
				if((dx == 1 && side == 2) || (dx == -1 && (side == 3 || side == 1 || side == 0)) || (dz == 1 && side == 5) || (dz == -1 && side == 4))
				{
					connectedSides = connectedSides | 1;
				} else if((dx == -1 && side == 2) || (dx == 1 && (side == 3 || side == 1 || side == 0)) || (dz == -1 && side == 5) || (dz == 1 && side == 4))
				{
	...
	public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity)
	{
		if(!world.isRemote)
		{
			...
			TileWallTeleporterBase teleporterBase = (TileWallTeleporterBase)tileEntity;
			if(teleporterBase.hasWTNetwork())
			{
				teleporterBase.getWTNetwork().entityCollided(entity);
			}
		}
